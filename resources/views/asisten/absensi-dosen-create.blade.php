...
@section('content')
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10 fade-in-up flex-1 w-full">
    <!-- Header Section -->
    <div class="flex items-center gap-4 mb-8">
        <a href="{{ route('asisten.absensi-dosen') }}" 
           class="inline-flex items-center justify-center w-12 h-12 bg-white border-2 border-gray-200 rounded-xl hover:bg-gray-50 transition-all duration-200">
            <i class="ph-fill ph-arrow-left text-xl text-gray-600"></i>
        </a>
        <div>
            <h1 class="text-3xl font-bold text-gray-900 mb-1">Tambah Absensi Dosen</h1>
            <p class="text-gray-600">Scan RFID atau masukkan email dosen untuk mencatat kehadiran</p>
        </div>
    </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div id="recentScans" class="p-6">
                    <p class="text-sm text-gray-500 text-center italic" id="noScanText">Belum ada scan</p>
                </div>
            </div>
        </div>
    </div>
...
function initAbsensiDosen() {
    ...
    const noScanText = document.getElementById('noScanText');
    ...
    async function loadAttendanceList(mkId, mkName) {
        ...
        try {
            const response = await fetch(`/asisten/absensi-dosen/list/${mkId}`);
    ...
    submitBtn.addEventListener('click', async () => {
        ...
        if (!matakuliah_id) { showStatus('error', 'Pilih mata kuliah terlebih dahulu!'); return; }
        if (!pertemuan) { showStatus('error', 'Pilih pertemuan terlebih dahulu!'); return; }
        if (!identifier) { showStatus('error', 'Scan RFID atau masukkan email dosen!'); identifierInput.focus(); return; }

        submitBtn.disabled = true;
        submitBtn.innerHTML = '<i class="ph-fill ph-spinner text-2xl animate-spin"></i><span class="text-lg">Memproses...</span>';

        try {
            const response = await fetch('{{ route("asisten.absensi-dosen.store.rfid") }}', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
                body: JSON.stringify({ identifier, matakuliah_id, pertemuan, materi, status })
            });
            ...
            if (data.success) {
                ...
                Toast.fire({ icon: 'success', title: data.message, text: data.nama });
                addRecentScan(data.nama, identifier);
                setTimeout(() => { window.location.href = '{{ route("asisten.absensi-dosen") }}'; }, 500);
            } else {
        ...
        if (type === 'success') {
            statusDisplay.innerHTML = `
                <div class="bg-gradient-to-r from-green-50 to-emerald-50 border-l-4 border-green-500 p-6 rounded-lg shadow-sm animate-pulse">
                    <div class="flex items-start gap-4">
                        <div class="bg-green-500 p-3 rounded-full"><i class="ph-fill ph-check-circle text-3xl text-white"></i></div>
                        <div class="flex-1">
            ...
            statusDisplay.innerHTML = `
                <div class="bg-gradient-to-r from-red-50 to-rose-50 border-l-4 border-red-500 p-6 rounded-lg shadow-sm">
                    <div class="flex items-start gap-4">
                        <div class="bg-red-500 p-3 rounded-full"><i class="ph-fill ph-warning-circle text-3xl text-white"></i></div>
                        <div class="flex-1">
    ...
    function addRecentScan(nama, identifier) {
        ...
        if (noScanText) { recentScans.innerHTML = scanItem; }
        else { recentScans.insertAdjacentHTML('afterbegin', scanItem); }
    }
}
</script>
@endpush

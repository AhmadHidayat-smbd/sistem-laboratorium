...
@section('content')
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-6 fade-in-up flex-1 w-full">
    <!-- Header Section -->
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div class="flex flex-col md:flex-row md:items-center gap-4">
            <div>
                <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight">Absensi Dosen</h2>
                <p class="text-gray-500 font-medium text-sm mt-1">Rekap kehadiran dosen mengajar per mata kuliah</p>
            </div>
            
            <!-- Tahun Ajaran Filter -->
            <form action="{{ route('asisten.absensi-dosen') }}" method="GET" class="flex items-center gap-2">
                <div class="relative">
        ...
        <div class="flex items-center gap-3">
            <button onclick="handleExport('{{ route('asisten.absensi-dosen.export') }}')"
               class="flex items-center gap-2 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-900 text-white rounded-xl font-semibold shadow-sm transition-all text-sm">
                <i class="ph-fill ph-file-xls text-white-600 text-xl"></i>
                <span>Export Excel</span>
            </button>

            <a href="{{ route('asisten.absensi-dosen.create') }}" 
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                            ...
                            <tr class="hover:bg-blue-50/30 transition-colors">
                                ...
                                <td class="px-6 py-3">
                                    <div class="flex items-center justify-center">
                                        <form action="{{ route('asisten.absensi-dosen.delete', $item->id) }}" method="POST" id="delete-absensi-{{ $item->id }}" class="inline m-0">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" 
            ...
            @else
                <div class="px-6 py-8 text-center text-gray-400">
                    <i class="ph-fill ph-clipboard-text text-3xl mb-2"></i>
                    <p class="text-sm font-semibold">Belum ada data absensi untuk mata kuliah ini.</p>
                </div>
            @endif
        </div>
    @empty
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-16 text-center">
            <i class="ph-fill ph-clipboard-text text-6xl text-gray-300 mb-4 block mx-auto"></i>

...
@section('page_title', 'Koreksi Data Kehadiran')
@section('page_subtitle', 'Modifikasi status kehadiran mahasiswa secara manual.')

@section('content')
<div class="max-w-4xl mx-auto animate-fade-in p-6">
    <!-- Back Button -->
    <a href="{{ route('dosen.absensi', ['matakuliah_id' => $matakuliah->id]) }}" class="inline-flex items-center gap-2 text-gray-500 hover:text-blue-600 font-bold mb-8 transition-colors group">
    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-10">
            <div class="flex items-center gap-4 mb-10">
                ...
                <div>
                    <h2 class="text-2xl font-black text-gray-800 tracking-tight">Koreksi Data</h2>
                    <p class="text-sm font-medium text-gray-400">Edit status kehadiran mahasiswa.</p>
                </div>
            </div>
            ...
            @if($pertemuanAda->isEmpty())
                <div class="py-12 text-center bg-amber-50 rounded-[2rem] border border-amber-100">
                    ...
                    <p class="text-amber-600/70 text-sm font-medium mb-8">Mahasiswa ini belum tercatat hadir dalam sesi manapun. Anda tidak dapat menambah data baru, hanya mengedit data yang sudah ada.</p>
                    <a href="{{ route('dosen.absensi', ['matakuliah_id' => $matakuliah->id]) }}" class="px-8 py-4 bg-amber-500 text-white rounded-2xl font-black hover:bg-amber-600 transition-all shadow-lg shadow-amber-100">
                        Kembali
                    </a>
                </div>
            @else
                <form method="POST" action="{{ route('dosen.absensi.update') }}" class="space-y-10">
                    @csrf
                    ...
                    <!-- Pertemuan -->
                    <div class="space-y-3">
                        <label class="block text-[11px] font-black text-gray-400 uppercase tracking-[2px] ml-2">Pilih Sesi Pertemuan (Hanya yang sudah ada)</label>
                    ...
                    <!-- Status -->
                    <div class="space-y-3">
                        <label class="block text-[11px] font-black text-gray-400 uppercase tracking-[2px] ml-2">Update Status Kehadiran</label>
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            <!-- Hadir -->
                            <label class="relative group cursor-pointer">
                                <input type="radio" name="status" value="Hadir" class="peer sr-only" required>
                                <div class="p-4 bg-white border-2 border-gray-100 rounded-2xl flex flex-col items-center justify-center gap-2 peer-checked:border-emerald-500 peer-checked:bg-emerald-50 transition-all duration-300 group-hover:border-emerald-200 h-full">
                                    ...
                                    <span class="font-black text-gray-700 peer-checked:text-emerald-900 text-sm">HADIR</span>
                                </div>
                            </label>
                            ...
                            <label class="relative group cursor-pointer">
                                ...
                                <div class="p-4 bg-white border-2 border-gray-100 rounded-2xl flex flex-col items-center justify-center gap-2 peer-checked:border-rose-500 peer-checked:bg-rose-50 transition-all duration-300 group-hover:border-rose-200 h-full">
                                    <div class="w-10 h-10 bg-rose-100 text-rose-600 rounded-full flex items-center justify-center">
                                        <i class="ph-bold ph-x text-xl"></i>
                                    </div>
                                    <span class="font-black text-gray-700 peer-checked:text-rose-900 text-sm">TIDAK HADIR</span>
                                </div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="pt-6 flex flex-col md:flex-row gap-4">
                        ...
                        <a href="{{ route('dosen.absensi', ['matakuliah_id' => $matakuliah->id]) }}" class="px-10 py-5 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-[1.5rem] font-bold transition-all text-center">
                            Batal
                        </a>
                    </div>
                </form>
